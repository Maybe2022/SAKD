...
class ResNet(nn.Module):
    ...
    def __init__(self, block, layers, num_classes=1000, zero_init_residual=False,
        ...
        #                                      detach_reset=True,v_threshold=0.)
        ...
        #                                      detach_reset=True,v_threshold=0.)
        # self.spike4 = neuron.MultiStepLIFNode(surrogate_function=surrogate.ATan(), backend='cupy', decay_input=False,
        #                                      detach_reset=True,v_threshold=0.)
        ...
        # self.bn1_ = nn.BatchNorm2d(64,track_running_stats=False, affine=False)
        #
        #
        ...
        #
        #
        # self.bn4 = nn.BatchNorm2d(512,track_running_stats=False, affine=False)
...
def mask(img):
    ...
    y1 = np.clip(y - h // 4, 0, h)
    y2 = np.clip(y + h // 4, 0, h)
    x1 = np.clip(x - h // 4, 0, w)
    x2 = np.clip(x + h // 4, 0, w)
...
def restore(x,T=4):
    B,C,H,W = x.shape
    x = x.reshape(B//T,T,C,H,W).mean(1)
    return x
...
def repeat(x,T=4):
    out = x.unsqueeze(0)
    out = out.repeat(T, 1, 1, 1, 1)
    return out

# qiu = resnet50(num_classes = 100)
